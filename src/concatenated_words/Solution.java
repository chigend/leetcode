...
public class Solution {
    public static void main(String[] args) {
        ...
        //        String[] words = {"f","m","h","q","u","n","nuqhmfj"};
    ...
    public List<String> findAllConcatenatedWordsInADict(String[] words) {
        ...
        for (String word : words) {
            if (isConcatenatedWord(word.toCharArray(), 0, 0)) {
                result.add(word);
            }
        }
        return result;
    ...
    public boolean isConcatenatedWord(char[] chs, int start, int count) {
        Node node = root;
        for (int i = start; i < chs.length; i++) {
            ...
            if (child.isWord) {
                //the last word return true
                if (i == chs.length - 1) {
                    return count >= 1;
                }
                //otherwise search dfs
                if (isConcatenatedWord(chs, i + 1, count + 1)) {
                    return true;
                }
            }
            node = child;
