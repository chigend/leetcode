...
public class Solution {
    ...
    public static void connect(TreeLinkNode root) {
        populateRight(root, null);
        System.out.print(root);
    }

    private static void populateRight(TreeLinkNode node, TreeLinkNode next) {
        if (node == null) return;
        node.next = next;
        populateRight(node.left, node.right);
        populateRight(node.right, null);
        if (next != null) populateRight(node.right, node.next.left);
    }
}
